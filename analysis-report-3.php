...
<body>
                        ...
                        <div class="nav-menu">
                            <nav class="mainmenu">
                                <ul>
                                   ...
                                   <li class="active"><a href="./analysis-report3.php">Analysis Report</a></li>   
    ...
    <div>
       <h1>Analysis Report 3: Total and average nights booked during each month.
       <a href="./analysis-report-2.php" class="previous round">&#8249;</a>
       <a href="./analysis-report-4.php" class="next round">&#8250;</a>
       <p>Total and average nights booked during each month.</p>
        </h1>
        <thead>
      ...
      <div class="row" style="border-radius:10px; display: flex; background-color: #F3EEE8; color: black; max-width: 80%; justify-content: center; align-items: center; margin: auto;">
        ...
        <p class="col-lg-2">Total Reservations Booked</p>
        <p class="col-lg-2">Total Nights Booked</p>
        <p class="col-lg-2">Average Nights Booked</p>
        </div>
        ...
        $sql = "
SELECT
    MONTHNAME(res.check_in_date) AS month,
    si.season_name,
    COUNT(DISTINCT res.reserve_id) AS total_reservations_booked,
    SUM(DATEDIFF(res.check_out_date, res.check_in_date)) AS total_nights_booked,
    SUM(DATEDIFF(res.check_out_date, res.check_in_date)) / COUNT(DISTINCT res.reserve_id) AS avg_nights_booked
FROM
    room_types_table rt
    ...
    JOIN season_info si ON MONTH(res.check_in_date) BETWEEN MONTH(si.start_date) AND MONTH(si.end_date)
WHERE
    YEAR(res.check_in_date) = 2023
GROUP BY
    MONTH(res.check_in_date),
    si.season_name
ORDER BY
    MONTH(res.check_in_date)";
...
try {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<p class="col-lg-2">' . $row['total_reservations_booked'] . '</p>';
        echo '<p class="col-lg-2">' . $row['total_nights_booked'] . '</p>';
        echo '<p class="col-lg-2">' . $row['avg_nights_booked'] . '</p>';
        echo '</div>';
    }
    echo '</table>';
